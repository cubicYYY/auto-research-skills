...
Copy `skills/paper-search/.env.example` to `.env` and fill in what you have.
...
see [`skills/paper-search/SKILL.md`](skills/paper-search/SKILL.md) for the
exact shell invocation, flag guidance, and output contract that Claude uses.
...
what you have, and the CLI loads it automatically via `python-dotenv`:
...
Runnable demos in [`skills/paper-search/examples/`](skills/paper-search/examples/):
...
| Claude can't find the skill                                | Confirm the skill loader is pointed at `skills/paper-search/` (not the repo root). |
...
See [`skills/paper-search/SKILL.md`](skills/paper-search/SKILL.md) for the
Claude invocation contract, and the top-level design notes (pipeline, dedupe
strategy, ranking formula, GitHub-source protocol) in
[`skills/paper-search/`](skills/paper-search/) source.
